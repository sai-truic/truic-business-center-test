...
const AdmitNewMembers: React.FC<Props> = ({ register, setValue, fetchedData }) => {
  const [admitMembers, setAdmitMembers] = useState<string>(fetchedData?.admitNewMembers || 'No');
  const [votingType, setVotingType] = useState<string>(fetchedData?.voteToAdmitNewMembers || 'Majority');
  const [isInfoModalOpen, setIsInfoModalOpen] = useState(false);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const quillRef = useRef<typeof ReactQuill>(null);

  const [contents, setContents] = useState({
    No: `The Company may not issue any additional LLC Units. The existing LLC Units may be transferred only in accordance with this Operating Agreement.`,
    YesMajority: `The Members may by <span class="text-[#F7931E] font-semibold">Majority</span> vote admit a new member in the Company and issue additional LLC Units to new members. Any new members shall, before being admitted and as a condition to admission, execute any document or documents required by the Company, agree to be and become a member of the Company, and agree to be bound by the terms of the Company Operating Agreement.`,
    YesUnanimous: `The Members may by <span class="text-[#F7931E] font-semibold">Unanimous</span> vote admit a new member in the Company and issue additional LLC Units to new members. Any new members shall, before being admitted and as a condition to admission, execute any document or documents required by the Company, agree to be and become a member of the Company, and agree to be bound by the terms of the Company Operating Agreement.`
  });

  const getContent = () => {
    if (admitMembers === 'No') return contents.No;
  ...
  useEffect(() => {
    setValue('admitNewMembers', admitMembers);
    setValue('voteToAdmitNewMembers', votingType);
    if (fetchedData?.pdfSections?.membershipChanges?.section_5_1_desc) {
      const fetchedContent = fetchedData.pdfSections.membershipChanges.section_5_1_desc;
      setContents(prevContents => ({
        ...prevContents,
        [admitMembers === 'No' ? 'No' : votingType === 'Majority' ? 'YesMajority' : 'YesUnanimous']: fetchedContent
      }));
    }
  ...
  useEffect(() => {
    setEditedContent(getContent());
    setValue('pdfSections.membershipChanges.section_5_1_desc', getContent(), {
      shouldValidate: true,
      shouldDirty: true,
    });
  }, [admitMembers])
  ...
  const handleVotingTypeChange = (newType: string) => {
    ...
    setValue('voteToAdmitNewMembers', newType);
  };
  ...
  const saveContent = () => {
    setContents(prevContents => ({
      ...prevContents,
      [admitMembers === 'No' ? 'No' : votingType === 'Majority' ? 'YesMajority' : 'YesUnanimous']: editedContent
    }));
    setValue('pdfSections.membershipChanges.section_5_1_desc', editedContent, {
      shouldValidate: true,
      shouldDirty: true,
    });
  ...
  const resetContent = () => {
    ...
    localStorage.removeItem('pdfSections.membershipChanges.section_5_1_desc_no');
    localStorage.removeItem('pdfSections.membershipChanges.section_5_1_desc_yes_majority');
    localStorage.removeItem('pdfSections.membershipChanges.section_5_1_desc_yes_unanimous');
    setEditedContent(getContent());
  };

  useEffect(() => {
    setEditedContent(getContent());
  }, [admitMembers, votingType, contents]);
          ...
          <div className="flex flex-col space-y-4 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex-1 space-y-2">
              <p className="text-neutral-950 text-base sm:text-lg md:text-xl font-normal italic">
                Decide whether your LLC should have the ability to bring in new members in the future.
              </p>
                ...
                <div className="flex items-center justify-center sm:justify-end mt-2 sm:mt-0">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      checked={admitMembers === 'Yes'}
                      onChange={(e) => {
                        const value = e.target.checked ? 'Yes' : 'No';
                        setAdmitMembers(value);
                        setValue('admitNewMembers', value);
                      }}
                    />
            ...
            >
              <div className="flex flex-col items-center space-y-4 sm:space-y-6">
                <p className="text-lg sm:text-xl md:text-2xl text-neutral-950 font-semibold text-center">
                  How should the LLC vote to admit new members?
                </p>
      ...
      <Dialog open={isInfoModalOpen} onClose={() => setIsInfoModalOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="mx-auto max-w-sm rounded-lg bg-white p-6 shadow-xl">
            <Dialog.Title className="text-lg font-medium text-neutral-950">Voting to Admit New Members</Dialog.Title>
            <Dialog.Description className="mt-2 text-sm text-gray-700">
              The LLC may add members by either a Majority vote (based on LLC Units) or a Unanimous vote (all owners must agree).
            </Dialog.Description>
            <button
              type="button"
...
export default AdmitNewMembers;
