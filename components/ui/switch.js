...
/*
This is how you can use the Switch component:

 <Switch id="darkMode" label="Dark Mode" />
*/

export const Switch = ({ id, label }) => {
  const { getState, updateState } = useInputState();
  
  const { checked = false } = getState('switch', id) || {};

  const handleChange = (e) => {
    updateState('switch', id, { checked: e.target.checked });
  };
