...
export const CustomDatePicker = ({ className, selected, onSelect, minDate, maxDate, icon, iconClassName, allowFutureDates = false }) => {
  ...
  const [date, setDate] = useState(selected ? selected !== "_DATE_" ? new Date(selected.split('T')[0]) : new Date(): new Date());
    ...
    if (selected) {
        if (selected !== "_DATE_") {
            ...
            if (typeof selected === 'string') {
              const [year, month, day] = selected.split('T')[0].split('-').map(Number);
              newDate = new Date(year, month - 1, day);
            } else if (selected instanceof Date) {
              newDate = new Date(selected.split('T')[0]);
            } else {
  ...
  return (
    <div className={`custom-date-picker relative ${className}`} ref={calendarRef}>
      <div className="relative space-x-4">
        {icon && (
          <span className={`absolute left-2 top-1/2 transform -translate-y-1/2 ${iconClassName}`}>
            {React.createElement(Icons[icon])}
          </span>
        )}
        <input
          className={`w-full text-sm bg-transparent sm:text-base font-medium text-gray-700 focus:outline-none transition-all duration-300 ease-in-out cursor-pointer ${icon ? 'pl-8' : ''}`}
