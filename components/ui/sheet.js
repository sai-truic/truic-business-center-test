...
/*
This is how you can use the Sheet component:
...
export const Sheet = ({ id, children, side = 'right' }) => {
  const { getState, updateState } = useInputState();
  
  const { isOpen = false } = getState('sheet', id) || {};

  const onClose = () => {
    updateState('sheet', id, { isOpen: false });
  };
